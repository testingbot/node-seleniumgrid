...
exports.findNode = function(desiredCapabilities, cb, blockPending) {
	...
	store.getAvailableNodes(function(sortedNodes) {
		...
		async.detectSeries(sortedNodes, function(node, callback) {
			...
			if (!foundNode) {
				log.info("No local nodes current available for these desired capabilities, forwarding to testingbot");
				
				return registry.getHubConfiguration(function(configString) {
					var config = JSON.parse(configString);
					if (config && config['useTestingBotFallback'] === true) {
						desiredCapabilities.client_key = config['testingbot_key'];
						desiredCapabilities.client_secret = config['testingbot_secret'];
			...
			foundNode.lastUsed = (new Date()).getTime();
